...
public class Kafka编程练习_消费者 {
    public static void main(String[] args) {
        // juc包是jdk中的并发编程的工具包，ConcurrentHashMap是线程安全的HashMap
        // 这里创建了一个hashmap来存储用户的guid和对应的用户名，保证key不重复
        ...
        timer.scheduleAtFixedRate(new StatisticTask(guidMap), 5, 10 * 1000);
    }
}
...
class StatisticTask extends TimerTask{
    ...
    @Override
    public void run() {
        System.out.println(DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss") + "，截止到当前的用户总数为：" + guidMap.size());
    }
}